...
# На вход функция принимает произвольное количество сортированных последовательностей (`iterables`),
...
# Вызываем функцию heapq_merge с аргументами *iterables (развернутый список iterables)
# и ключом key=lambda x: x.
# Функция heapq_merge принимает несколько отсортированных итерируемых объектов
# и возвращает отсортированный итератор, объединяя элементы из всех итерируемых объектов.
    return heapq_merge(*iterables, key=lambda x: x)
...
#  Проверяем, что результат работы функции merge, к которой передаются аргументы развернутого списка data
#  и ключ key=itemgetter(0), равен ожидаемому результату reference.
assert list(merge(*data, key=itemgetter(0))) == reference

print(list(merge(*data, key=itemgetter(0))))
...
assert merge(*data, key=lambda x: x[0]) == reference

print(merge(*data, key=lambda x: x[0]))